...
import { fetchWorkers } from '../../../shared/services/api';
...
const Users: React.FC = () => {
  ...
  const [workersSortKey, setWorkersSortKey] = useState<'id'|'name'|'ic'|'contact'|'position'|'status'>('id');
  ...
  const displayedWorkers = useMemo(() => {
    ...
    return list;
  }, [workers, workerQuery, workersSortKey, workersSortDir]);
        ...
        <div className="companies-list" style={{ marginBottom: 12 }}>
          ...
          <div className="permit-status-item" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            ...
            <div style={{ display: 'flex', gap: 8 }}>
              <button className="manage-btn" onClick={() => { loadWorkersForCompany(null, null); setSelectedCompanyId(null); setSelectedCompanyName(null); }}>View</button>
            </div>
          </div>
          ...
          {companiesFiltered.length === 0 ? (
            ...
            <div key={c.id} className="permit-status-item" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              ...
              <div style={{ display: 'flex', gap: 8 }}>
                <button className="manage-btn" onClick={() => loadWorkersForCompany(c.id, c.name)}>View</button>
              </div>
            </div>
          ))}
        </div>
        ...
        <div className="card-border">
          <div className="card-scroll">
            <table className="users-table">
              ...
              <tbody>
                {workersLoading ? (
                  <tr><td colSpan={6} style={{ padding: 16 }}>Loading workers…</td></tr>
                ) : workersError ? (
                  <tr><td colSpan={6} style={{ padding: 16, color: 'red' }}>Error: {workersError}</td></tr>
                ) : (displayedWorkers || []).length === 0 ? (
                  <tr><td colSpan={6} style={{ padding: 16 }}>No workers found.</td></tr>
                ) : (
